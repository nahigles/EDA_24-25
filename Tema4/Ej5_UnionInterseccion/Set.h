...
public:
    ...
    // Complejidad lineal en n+m
    // T(n+m) --> O(n+m) , siendo n el numero de elementos del set1 y m el numero de elementos del set2
    // Tiene que recorrer los dos sets enteros
    Set<T> operator||(const Set<T>& other) const { // Uni´on de conjuntos
        Set<T> unionSet = Set<T>(this->nelems + other.nelems);
        int i = 0;
        int j = 0;

        while (i < this->nelems && j < other.nelems) {
            if (this->array[i] == other.array[j]) {
                unionSet.array[unionSet.nelems] = this->array[i];
                unionSet.nelems++;
                i++;
                j++;
            }
                ...
                unionSet.array[unionSet.nelems] = other.array[j];
                unionSet.nelems++;
                j++;
            }
        }
    ...
    // T(n) --> O(n), siendo n el numero de elementos del array set mas pequenyo
    // Itera solo n veces
    Set<T> operator&&(const Set<T>& other) const { // Intersecci´on de conjuntos
        // Como max tamnyo del set mas pequenyo
        Set<T> intersectionSet = Set<T>(std::min(this->nelems, other.nelems));
        ...
        while (i < this->nelems && j < other.nelems) {
            ...
            // Si son iguales añado
            else {
                intersectionSet.array[intersectionSet.nelems] = this->array[i];
