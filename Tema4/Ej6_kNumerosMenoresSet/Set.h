...
public:
    ...
    // Devuelve valor maximo del conjunto. O(1)
    T getMax() const {
        return this->array[nelems - 1];
    }

    // Devuelve valor minimo del conjunto. O(1)
    T getMin() const {
        return this->array[0];
    }

    // Elimina valor maximo del conjunto. O(1)
    void removeMax() {
        nelems--;
    }

    // Elimina valor minimo del conjunto. O(n-1), siendo n el numero de elementos del array
    void removeMin() {
        shiftLeftFrom(0);
        this->nelems--;
    }
