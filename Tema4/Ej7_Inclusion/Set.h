...
public:
    ...
    // La funcion tiene complejidad lineal
    // T(n) --> O(n) siendo n el número de elementos del set que menos elementos tenga
    bool operator<=(const Set<T>& set) const {
        int i = 0;
        int j = 0;
        while (i < this->nelems && j < set.nelems) {
