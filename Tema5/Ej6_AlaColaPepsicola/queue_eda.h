...
public:
   ...
   // Cuela los elementos de la cola b detras del elemento a
   // Complejidad lineal en n, sieindo n el numero de elementos de la cola
   void cuela(const T a, queue<T>& b) {
       ...
       if (nelems) {
           ...
           Nodo* aux = this->prim;
           while (aux != nullptr && aux->elem != a) // O(n), n = nelems
               aux = aux->sig;
